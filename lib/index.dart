// Export pages
export '/pages/inicial/inicial_widget.dart' show InicialWidget;
export '/usuario/login/login_widget.dart' show LoginWidget;
export '/pages/delivery/delivery_widget.dart' show DeliveryWidget;
export '/pages/restaurantes/restaurantes_widget.dart' show RestaurantesWidget;
export '/pages/paginas_de_acao/detalhe_pedidos/detalhe_pedidos_widget.dart'
    show DetalhePedidosWidget;
export '/pages/paginas_de_acao/carrinho/carrinho_widget.dart'
    show CarrinhoWidget;
export '/usuario/config/config_widget.dart' show ConfigWidget;
export '/usuario/cadastro_endereco/cadastro_endereco_widget.dart'
    show CadastroEnderecoWidget;
export '/usuario/meusenderecos/meusenderecos_widget.dart'
    show MeusenderecosWidget;
export '/usuario/editar_endereco/editar_endereco_widget.dart'
    show EditarEnderecoWidget;
export '/pages/ordem_aceita/ordem_aceita_widget.dart' show OrdemAceitaWidget;
export '/pages/paginas_de_acao/pagamento/pagamento_widget.dart'
    show PagamentoWidget;
export '/pages/endereco_entrega/endereco_entrega_widget.dart'
    show EnderecoEntregaWidget;
export '/pages/filtro/filtro_widget.dart' show FiltroWidget;
